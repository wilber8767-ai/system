...
const bigInputBase: CSSProperties = {
  ...
  fontSize: 28, fontWeight: 800, color: C.slate800,
  ...
  return (
    <div style={{ position:"relative", display:"flex", alignItems:"center" }}>
      ...
      <input type="number" value={value} placeholder={placeholder}
        ...
        style={{
          ...bigInputBase,
          paddingLeft: prefix?36:16, paddingRight: suffix?52:16,
      ...
      />
      {suffix && <span style={{ position:"absolute", right:14, color:C.slate400, fontWeight:600, fontSize:14, pointerEvents:"none", whiteSpace:"nowrap" }}>{suffix}</span>}
    </div>
  );
}
  ...
  return (
    <div style={{ position:"relative" }}>
      <select value={value} onChange={e=>onChange(e.target.value)}
        onFocus={()=>setF(true)} onBlur={()=>setF(false)}
        style={{ ...textInputBase, paddingRight:40, cursor:"pointer", borderColor:f?C.indigo:C.slate200, boxShadow:f?"0 0 0 4px rgba(79,70,229,0.18)":"none" }}
      >
...
// ── Field helper ──────────────────────────────────────────────────────────────
function Field({ label, children, highlight=false }: { label:React.ReactNode; children:React.ReactNode; highlight?:boolean }) {
  return (
    <div>
      <label style={{ ...labelStyle, ...(highlight ? { display:"flex", alignItems:"center", gap:6 } : {}) }}>{label}</label>
      {children}
    </div>
  );
}
...
function ReportPage({ client, prot, onBack }: { client:ClientInfo; prot:ProtectionData; onBack:()=>void }) {
  ...
  // 保障摘要列表（報告用）
  const coverageRows = [
    { label:"住院定額",       val:`${nv(prot.medicalCoverage.hospitalDaily).toLocaleString("zh-TW")} 元/日` },
    { label:"住院實支",       val:`${nv(prot.medicalCoverage.hospitalReal).toLocaleString("zh-TW")} 元/日` },
    { label:"手術定額",       val:`${fmt(nv(prot.medicalCoverage.surgeryLump)*10000)} 元` },
    { label:"手術實支",       val:`${fmt(nv(prot.medicalCoverage.surgeryReal)*10000)} 元` },
    { label:"醫療雜費",       val:`${fmt(nv(prot.medicalCoverage.medicalMisc)*10000)} 元` },
    { label:"壽險身故",       val:`${fmt(nv(prot.lifeInsurance)*10000)} 元` },
    { label:"意外身故",       val:`${fmt(nv(prot.accidentDeath)*10000)} 元` },
    { label:"意外實支",       val:`${fmt(nv(prot.accidentReal)*10000)} 元` },
    { label:"意外住院日額",   val:`${nv(prot.accidentHospitalDaily).toLocaleString("zh-TW")} 元/日` },
    { label:"重大傷病一次金", val:`${fmt(nv(prot.criticalIllness)*10000)} 元` },
    { label:"癌症一次金",     val:`${fmt(nv(prot.cancerLumpsum)*10000)} 元` },
    { label:"化/放療補助金",  val:`${nv(prot.cancerChemoDaily).toLocaleString("zh-TW")} 元/日` },
    { label:"長照一次金",     val:`${fmt(nv(prot.ltcLumpsum)*10000)} 元` },
    { label:"長照月扶助金",   val:`${nv(prot.ltcMonthly).toLocaleString("zh-TW")} 元/月` },
  ].filter(r => r.val.replace(/[^\d]/g,"") !== "0" && r.val.replace(/[^\d]/g,"") !== "");
    ...
    <div style={{ minHeight:"100vh", background:C.slate900 }}>
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"28px 24px", display:"flex", flexDirection:"column", gap:22 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:18 }}>
          ...
          {/* Risk gaps — now includes 4 items */}
          <div style={{ background:C.slate800, borderRadius:20, padding:22, border:`1px solid ${C.slate700}` }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:18 }}>
            ...
            {[
              ...
              <div key={r.label} style={{ background:"rgba(255,255,255,0.05)", borderRadius:12, padding:"12px 14px", marginBottom:10 }}>
                ...
                <div style={{ fontSize:22, fontWeight:900, color:r.gap>0?"#fb7185":"#34d399" }}>
                  {r.gap>0 ? `-${r.gap>=10000?fmt(r.gap):r.gap.toLocaleString("zh-TW")} ${r.unit}` : "已足備"}
                </div>
  ...
  const protCards = [
    {
      ...
      content:(
        <>
          ...
          <Field label={<><span style={{color:C.rose}}>●</span> 醫療雜費（萬）<span style={{color:C.rose,fontSize:13,fontWeight:700,marginLeft:6}}>← 關鍵</span></>}>
